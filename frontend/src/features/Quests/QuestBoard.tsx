...
export const QuestBoard: React.FC<{
  ...
  const handleOpenProofModal = (quest: Quest, _e?: React.MouseEvent) => {
    soundEngine.play('click');
  ...
  const handleConfirmCompletion = async (confirmed: boolean) => {
    ...
    if (!confirmed || !questForProof) return;
      ...
      {questForProof && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
          <motion.div
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="w-full max-w-lg max-h-[90vh] overflow-y-auto game-panel-gold rounded-3xl p-6 border-2 border-amber-500/60 shadow-[0_0_50px_rgba(245,158,11,0.3)] text-white space-y-4 relative"
          >
            {/* Confirmation Dialog Popover Overlay */}
            {showConfirmDialog && (
              <div className="absolute inset-0 z-50 bg-black/90 backdrop-blur-md rounded-3xl p-6 flex flex-col items-center justify-center text-center space-y-4 animate-in fade-in zoom-in-95 duration-200">
                ...
                <p className="text-xs font-mono text-zinc-300 max-w-xs">
                  Confirming will award XP, Gold, and Trophies to your account. Fake verification breaks anti-cheat integrity.
                </p>
                <div className="flex items-center justify-center gap-4 pt-2 w-full max-w-xs">
                  <button
                    onClick={() => handleConfirmCompletion(false)}
                    className="flex-1 py-3 px-4 rounded-xl border border-white/10 hover:border-zinc-400 text-zinc-400 hover:text-white font-mono text-xs font-bold uppercase transition-all"
                  >
                    NOT TRUE (CANCEL)
                  </button>
                  <button
                    onClick={() => handleConfirmCompletion(true)}
                    className="flex-1 py-3 px-4 rounded-xl bg-amber-500 hover:bg-amber-400 text-zinc-950 font-mono text-xs font-black uppercase transition-all shadow-[0_0_20px_rgba(245,158,11,0.4)]"
                  >
                    TRUE (OK)
                  </button>
                </div>
